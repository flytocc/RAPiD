...
from utils.utils import print_rank, get_world_size, is_main_process, synchronize
import api
...
if __name__ == '__main__':
    ...
    print_rank(f'effective batch size = {batch_size} * {subdivision}')
    # dataset setting
    print_rank('initialing dataloader...')
    if args.dataset == 'COCO':
        train_img_dir = '/home/user/Database/MSCOCO2017/train2017'
        assert 'COCO' in train_img_dir # issue #11
        train_json = '/home/user/Database/MSCOCO2017/annotations/instances_train2017.json'
        ...
        def burnin_schedule(i):
            ...
            if i < burn_in:
                ...
                factor = 0.1
            return factor
    elif args.dataset == 'POVL_RBB':
        POVL_ROOT = "/home/user/Database/B/POVL/"
        train_img_dir = [
            POVL_ROOT + "images/",
            POVL_ROOT + "images/",
            POVL_ROOT + "images/",
        ...
        train_json = [
            POVL_ROOT + "annotations/povl_v0.54_good_rbb.json",  # 24813
            POVL_ROOT + "annotations/povl_v0.54_normal_rbb.json",  # 1800
            POVL_ROOT + "annotations/povl_v0.54_bad_rbb.json",  # 6180
            # POVL_ROOT + "annotations/povl_P2_v0.54_rbb.json",  # 11219
        ]
        val_img_dir = POVL_ROOT + "images_P3/"
        val_json = POVL_ROOT + 'annotations/povl_P3_v0.54_rbb.json'  # 6900
        lr_SGD = 0.0001 / batch_size / subdivision
        # Learning rate setup
        def burnin_schedule(i):
            burn_in = 500
    ...
    start_iter = -1
    if args.checkpoint:
        print_rank("loading ckpt...", args.checkpoint)
    ...
    elif args.dataset in {'MW', 'HBCP', 'HBMW', 'CPMW', 'POVL_RBB'}:
    ...
    if args.dataset not in args.checkpoint:
        ...
        print_rank(f'begin from iteration: {start_iter}')
    ...
    for iter_i in range(start_iter, 100000):
        ...
        if iter_i % args.print_interval == 0:
            ...
            print_rank(f'[Iteration {iter_i}] [learning rate {current_lr:.3g}]',
                  f'[Total loss {loss:.2f}] [img size {dataset.img_size}]')
            if distributed:
                loss_str = model.module.loss_str
            else:
                loss_str = model.loss_str
            print_rank(loss_str)
            max_cuda = torch.cuda.max_memory_allocated(0) / 1024 / 1024 / 1024
            print_rank(f'Max GPU memory usage: {max_cuda:.4f} GigaBytes')
        ...
        if iter_i > 0 and (iter_i % args.checkpoint_interval == 0):
            if is_main_process():
                ...
                print_rank(f"save model: {save_path}")
